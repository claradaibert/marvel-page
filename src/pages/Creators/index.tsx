...
const Creators: React.FC = () => {
  ...
  const handleSeeMoreClick = async () => {
    ...
    try {
      ...
      const response = await api.get(
        `/creators?${requestSearchValue}offset=${requestOffset}&apikey=${myPublicKey}`
      );
  ...
  useEffect(() => {
    const handleSearch = async (e: string) => {
      try {
        const response = await api.get(
          `/creators?nameStartsWith=${e}&apikey=${myPublicKey}`
        );
  ...
  useEffect(() => {
    const getList = async () => {
      try {
        const response = await api.get(`/creators?apikey=${myPublicKey}`);
        setPageList(response.data.data.results);
        setTotalItems(response.data.data.total);
      } catch (err: any) {
        toast.error(err?.response?.data?.message);
